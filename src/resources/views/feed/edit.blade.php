    ...
    <div class="row">
        <div class="col-lg-12 margin-tb">
            ...
            <div class="pull-right">
                <a class="btn btn-primary" href="{{ route('feed.index') }}" enctype="multipart/form-data">
                    Back</a>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('feed.update',$feed->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    <strong>Url:</strong>
                    <input disabled type="text" name="url" value="{{ $feed->url }}" class="form-control" placeholder="Url">
                    @error('url')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="status">
                        @foreach (\App\Models\Feed::getArrrayStatus() as $key => $value)
                            <option value="{{ $key }}" {{$feed->status == $key ? 'selected="selected"' : ''}}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
